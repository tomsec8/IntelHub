...
document.addEventListener('DOMContentLoaded', () => {
    ...
    } else if (fileType === 'hash') {
        fileInput.accept = '*/*';
        prompt.textContent = "Select any file to calculate its hashes.";
    }
    ...
    fileInput.addEventListener('change', (event) => {
        ...
        switch (fileType) {
            ...
            case 'hash': processPromise = handleHashFile(file); break;
        }
...
async function handleHashFile(file) {
    const buffer = await file.arrayBuffer();
    try {
        const title = `Hashes for ${file.name}`;
        
        async function computeHashAlgo(algorithm, buf) {
        ...
        const [sha1, sha256, sha512] = await Promise.all([
            computeHashAlgo('SHA-1', buffer),
            computeHashAlgo('SHA-256', buffer),
            computeHashAlgo('SHA-512', buffer)
        ]);
        ...
        return createResultsTable(title, dataObject, 'Algorithm', 'Hash Value');
    } catch (e) {
        const p = document.createElement('p');
        p.textContent = 'Error: Could not calculate hashes.';
        return p;
    }
}
