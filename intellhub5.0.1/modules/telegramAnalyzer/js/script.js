...
function getMessagesPerPage() {
    ...
    return parseInt(pageSizeFilter.value, 10) || 100;
}
...
function transformTelegramExport(data) {
    ...
    data.chats.list.forEach(chat => {
        (chat.messages || []).forEach(msg => {
            ...
            if (msg.photo) {
                mediaObject = { photo: true };
            } else if (msg.file) {
                mediaObject = { document: { mime_type: msg.mime_type || '' } };
            } else if (msg.contact_information) {
                mediaObject = { contact: true };
            } else if (msg.mediaKind && typeof msg.mediaKind === 'number') {
                mediaObject = { generic: true, kind: msg.mediaKind };
            }
...
function analyzeData(data) {
    ...
    document.getElementById('mediaOnlyFilter').onchange = () => filterTimeline();
...
function filterTimeline() {
    ...
    const mediaOnly = document.getElementById('mediaOnlyFilter').checked;
    ...
    if (mediaOnly) {
        filteredMessages = filteredMessages.filter(m => m.media || (m.mediaKind && typeof m.mediaKind === 'number'));
    }
