...
function renderProfilerResults(text) {
    ...
    const resultArea = document.getElementById("profiler-result-area");
...
export function restoreTextProfilerView(container, state) {
    if (state.mainOpen) {
        const profilerBtn = Array.from(container.querySelectorAll('.category-button')).find(btn => btn.textContent === "Text Profiler");
        if (profilerBtn) {
    ...
    if (state.analyzedText) {
        renderProfilerResults(state.analyzedText);
    }
}

export function initializeTextProfiler(container) {
    function saveProfilerState() {
        const isMainOpen = profilerWrapper.classList.contains("open");
        if (!isMainOpen) {
            resetViewState();
            return;
        }
    ...
    const { wrapper: profilerWrapper } = createSection(container, "Text Profiler", {
        onToggle: (isOpen) => {
            if (!isOpen) {
                const resultArea = document.getElementById("profiler-result-area");
    ...
    pasteBtn.addEventListener("click", () => {
        ...
        if (userInput) {
            renderProfilerResults(userInput);
            saveProfilerState();
        }
    });
    ...
    analyzePageBtn.addEventListener("click", async () => {
        ...
        if (tab.url.startsWith('chrome://') || tab.url.startsWith('https://chrome.google.com/')) {
            flashButton(analyzePageBtn, "Restricted URL", true);
            return;
        }
        ...
        try {
            ...
            flashButton(analyzePageBtn, "Error", true);
        }
    });
