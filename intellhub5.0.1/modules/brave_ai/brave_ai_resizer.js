...
document.addEventListener('click', function (e) {
    ...
    if (link && link.href && link.href.startsWith('http')) {
        try {
            ...
            // Allow internal Brave Search/Ask links to work normally
            if (url.hostname.includes('brave.com') && url.pathname.startsWith('/ask')) {
                return; // Let Brave handle it
            }

            // For all other links, force a new tab
            e.preventDefault();
            e.stopPropagation();
...
window.addEventListener('wheel', function(e) {
    const isTextInput = e.target.closest('textarea, input, [contenteditable="true"], [role="textbox"]');
    if (!isTextInput && Math.abs(e.deltaY) > Math.abs(e.deltaX)) {
        e.preventDefault();
        e.stopPropagation();
        
        window.parent.postMessage({
            type: 'FORCE_SCROLL_PARENT',
            deltaY: e.deltaY
        }, '*');
    }
